...
function MessagesTab() {
  ...
  function handleEdit() {
    const message = messages.find(e => e.id === selectedMessageId);
    setEditForm({ ...message });
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from('messages').delete().eq('id', selectedMessageId);
    if (error) {
  ...
  async function handleToggleBlock() {
    const message = messages.find(e => e.id === selectedMessageId);
    if (!message) return;
    const newStatus = message.status === 'blocked' ? 'active' : 'blocked';
    const { error } = await supabase
      .from('messages')
  ...
  const columns = messages[0] ? Object.keys(messages[0]) : [];

  // Filtrado frontend
  const filteredMessages = messages.filter(message =>
    columns.some(col =>
      (message[col] || '').toString().toLowerCase().includes(search.toLowerCase())
    )
  );
      ...
      <table className="admin-table">
        ...
        <tbody>
          {pagedMessages.map(message => (
            <tr
              key={message.id}
              className={message.id === selectedMessageId ? 'selected-row' : ''}
