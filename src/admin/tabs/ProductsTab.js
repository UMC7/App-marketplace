...
function ProductsTab() {
  ...
  function handleEdit() {
    const product = products.find(e => e.id === selectedProductId);
    setEditForm({ ...product });
  ...
  const filteredProducts = products.filter(product =>
    columns.some(col =>
      (product[col] || '').toString().toLowerCase().includes(search.toLowerCase())
    )
  );
